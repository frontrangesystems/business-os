...
**Date:** 2026-07-25
**Status:** Proposed — awaiting sign-off. **No code changed yet.**
**Owner:** Matt + Claude
...
1. **Quantity extraction** — the current engine does NOT capture quantity, only description/code/unit. A "takeoff" product presumably needs quantity. Add it? (Recommended: yes.)
2. **Package name** for the engine library — e.g. `@frontrangesystems/business-os-extraction-engine`, `-pdf-takeoff`, `-doc-parser-core`. New top-level package (flagging per repo convention).
3. **Search in the pilot?** Include page-text search (the `document_parser_pages` table + FTS), or keep the pilot to upload → extracted takeoff → export only? (Leaner = faster to ship.)
4. **CSV export format** — what columns/shape do estimators want the takeoff in (does it need to match a specific estimating tool's import)?
